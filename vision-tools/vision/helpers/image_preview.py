...
import vision.helpers.display




########################################################
########### SOURCE: cvlab.view.image_preview ###########
########################################################
...
class PreviewScrollArea(QScrollArea):
    ...
    def mouseMoveEvent(self, event):
        ...
        if event.buttons() == Qt.LeftButton:
            if self.lastPos and self.lastPos != event.pos():
            ...
            if self.lastPos:
                self.parent().move(event.globalPos() - self.lastPos)
            else:
                self.lastPos = event.globalPos() - self.parent().pos()
...
class PreviewWindow(QFrame):
    ...
    def __init__(self, manager, name, image=None, message=None, position=None, size=None, high_quality=False):
        super(PreviewWindow, self).__init__(None, Qt.Dialog)
    ...
    def contextMenuEvent(self, event):
        ...
        raise_ = menu.addAction("Raise windows")
...
class WindowManager(QObject):
    ...
    def waitKeyMouse(self, delay=0):
        if not delay or delay <= 0: timeout = 360000000
