...
@perform_randomly
class AdjustDarkness(Operation):
    """ Adjust global darkness if under given threshold. """
    def __init__(self, threshold=100):
        self._threshold = threshold
        self._extra_brightness = (30, 80)

    def apply_on_image(self, image):
        if np.average(image) > self._threshold:
            return image

        hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
        value = random.randint(*self._extra_brightness)
...
@perform_randomly
class AddBrightness(Operation):
    def __init__(self, value=90, dev=10):
        self._value = value
        self._dev = dev

    def apply_on_image(self, image):
        val = np.array([self._value + random.randint(0, self._dev)], dtype=np.float)
        return cv2.add(image, val)


@perform_randomly
class AddDarkness(Operation):
    def __init__(self, value=90, dev=10):
        self._value = value
        self._dev = dev
