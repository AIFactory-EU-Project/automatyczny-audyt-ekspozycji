"""
input: 172.16.20.252-a.json  172.16.20.252-a.png


input: image_after_transformations & camera_ip
1) camera_ip -> shop_id, segment_id
2) retrive boxes from ai_service
3) read shop planogram from db by shop_id, segment_id

```
select row_to_json(t) from (
    select index, shelf, position, faces_count from planogram_element inner join sku on (planogram_element.sku_id = sku.id) where planogram_element.planogram_id=(
        select planogram_id from shop_planogram_assignment where shop_id=20 and segment_id=2
    )
) t;
```
