""" Generates synthetic dummy data with usage of real data backgrounds. """
import json
import os
import cv2
...
from ocr.aug.augmenter.dummy_augmenter import DummyTextAugmenter
from ocr.aug.config import config


DST = "/tytan/raid/neuca/data/aocr/augs/dummy/v1"


def generate_image(augmenter, iteration, set_dir, set_labels):
    img, img_value = augmenter.get_synthetic_dummy_text()
    if img is None:
        return
...
def generate(dst):
    ...
    dummy_aug = DummyTextAugmenter()

    train_iterations = int(config.params.dummy_gen_iterations * config.params.train_ratio)
    val_iterations = config.params.dummy_gen_iterations - train_iterations
    for set_name, iterations in zip(["train", "val"], [train_iterations, val_iterations]):
        ...
        def gen(iteration):
            generate_image(dummy_aug, iteration, set_dir, set_labels)
